 ...
 * attributes), found with the access token.
 *
 * @author Jerome Leleu
 * @since 3.5.0
 */
public class OAuth20ProfileController extends BaseOAuthWrapperController {
    ...
    protected ResponseEntity<String> handleRequestInternal(final HttpServletRequest request,
        ...
        map.put(AUTHORITIES, Arrays.asList(new String[] { "ROLE_USER" }));
        final String value = OAuthUtils.jsonify(map);
     ...
     *             the io exception
     */
    protected Map<String, Object> writeOutProfileResponse(final Authentication authentication, final Principal principal)
            throws IOException {
